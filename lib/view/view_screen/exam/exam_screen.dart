...
import '../../view_dialog/exam_pause_dialog.dart';
import '../../view_dialog/exam_quit_dialog.dart';
...
import '../../view_tab/exam/exam_question_gridins_item.dart';
import '../../view_tab/exam/exam_question_item.dart';

// ignore: must_be_immutable
class ExamScreen extends BasePage {
  ExamListQuestion? examObject;
  ExamJSONObject? examQuestionObject;
  int timeRemain;
  int posResume;

  ExamScreen(this.examObject, this.examQuestionObject,
      {this.timeRemain = 0, this.posResume = 0, super.key});

  @override
  BasePageState<BasePage> createState() => _State();
}

class _State extends BasePageState<ExamScreen> with WidgetsBindingObserver {
  ...
  void initState() {
    ...
    timeCountdown.value =
        widget.timeRemain > 0 ? (widget.timeRemain * 60) : (134 * 60);
    _startTimer();

    if (widget.posResume > 0 && widget.posResume < questionSize) {
      Future.delayed(Duration.zero, () async {
        _pageController.jumpToPage(widget.posResume);
      });
    }
                      ...
                      child: Row(children: [
                        BounceWidget(
                            ...
                            onPress: () {
                              ...
                              ExamQuitDialog.show(context, (isSave) {
                                if (isSave) {
                                  _handleSave().then((value) {
                          ...
                          child: Container(
                            ...
                            child: Stack(
                              ...
                              children: [
                                ...
                                Padding(
                                  padding: EdgeInsets.only(bottom: 6.0.dp()),
                                  child: ValueListenableBuilder(
                                    valueListenable: timeCountdown,
                                    builder: (context, time, child) {
                                      return Text(
                                        _timeString(time),
                                        style: UIFont.fontAppBold(13.0.sp(),
      ...
      children: [
        for (var indexPage = 0;
          ...
          Utils.checkGridIns(questionsList![indexPage].question?.kindId ?? "")
              ? ExamQuestionGridInsItem(indexPage, questionsList![indexPage],
                  currentQuestion, _setAnswerChooseListener)
              : ExamQuestionItem(indexPage, questionsList![indexPage],
                  currentQuestion, _setAnswerChooseListener)
        ],
      ],
    );
  ...
  Future<void> _handleSave({bool isComplete = false}) async {
    final timeRemaining = isComplete ? 0 : (timeCountdown.value ~/ 60 + 1);
    ...
    final examStateObject = ExamStateObject(widget.examObject?.id,
        timeRemaining, yourAnswerMap, writingReadingScore + mathScore);
