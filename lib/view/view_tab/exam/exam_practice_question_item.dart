...
class ExamPracticeQuestionItem extends BasePage {
  int posQuestion;
  ExamCellObject? examObject;
  int currentQuestion;
  Function(int currentQuestion, bool isNextQuestion)? setAnswerChooseListener;

  ExamPracticeQuestionItem(this.posQuestion, this.examObject,
...
class _State extends BasePageState<ExamPracticeQuestionItem>
  ...
  void initState() {
    ...
    currentQuestionChild = _checkQuestionNotChoose(questionObject?.content);
  ...
  Widget build(BuildContext context) {
    ...
    return Column(children: [
      ...
      if (isChoiceAnswerBottom)
        _answerBottomView(questionObject?.content ?? [])
    ]);
  }
  ...
  Widget _questionContentView() {
    ...
    final gText =
        questionObject?.general?.gText?.replaceAll("\n", "<br>");
  ...
  Widget _answerTopItem(int index, QuestionContent contentObject, String answer,
    ...
    return Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
      GestureDetector(
        onTap: () {
          if (yourAnswer > 0 || isChoiceAnswerBottom) return;
